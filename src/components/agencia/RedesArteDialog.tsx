  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        ...
        <div className="max-h-screen space-y-6 overflow-y-auto pr-2">
          <section className="space-y-3">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <div><Label>Cores da marca</Label><Input value={cores} onChange={(event) => setCores(event.target.value)} placeholder="Cole os códigos das cores, separados por vírgula" /></div>
          ...
          {aprovada && urlAprovada && <section className="space-y-4 border-t border-border pt-4">
            ...
            <div ref={previaRef} className={`relative mx-auto w-full max-w-md overflow-hidden rounded-md border border-border bg-muted ${FORMATOS[formato].classe}`}><img src={urlAprovada} alt="Arte em edição" className="absolute h-full w-full object-cover" /><div className="absolute inset-x-0 bottom-0 bg-background/80 p-6"><p className={`text-xl font-bold text-foreground ${alinhamento === 'centro' ? 'text-center' : 'text-left'}`}>{textoAtual}</p></div></div>
